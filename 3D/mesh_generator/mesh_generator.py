...
class MeshGeneratorBuilder:
    ...
    def build(self) -> MeshGenerator:
        if self._geometry.width < self._geometry.die_width or self._geometry.depth < self._geometry.die_depth:
...
class MeshGenerator:
    ...
    def generate_grid(self) -> Grid:
        print(f"Generating 3D mesh: {self.geo.nx}x{self.geo.ny}x{self.geo.nz} elements...")
        ...
        print(f" - Paste:    {idx_ihs_end} to {idx_paste_end - 1} \t({n_paste} layers) -> Pattern: {self.pattern}")
        print(f" - Heatsink: {idx_paste_end} to {self.geo.nz - 1} \t({n_heatsink} layers)")

        if n_heatsink < 0:
            raise ValueError("Error: Layer configuration results in negative heatsink layers. Increase nz.")
        ...
        elements = []
        node_id_counter = 1
        nodes_map = np.zeros((self.geo.nx + 1, self.geo.ny + 1, self.geo.nz + 1), dtype=int)
        ...
        for k in range(self.geo.nz + 1):
            for j in range(self.geo.ny + 1):
                for i in range(self.geo.nx + 1):
                    ...
                    if is_top or (in_radiator_zone and ENTIRE_RADIATOR_HAS_DERICHLET_BC):
        ...
        for k in range(self.geo.nz):
            for j in range(self.geo.ny):
                for i in range(self.geo.nx):
                    n_ids = [
                        nodes_map[i,   j,   k],   nodes_map[i+1, j,   k],
                        nodes_map[i+1, j+1, k],   nodes_map[i,   j+1, k],
                        nodes_map[i,   j,   k+1], nodes_map[i+1, j,   k+1],
                        nodes_map[i+1, j+1, k+1], nodes_map[i,   j+1, k+1]
                    ]
                    ...
                    if k < idx_silicon_end:
                        if self._is_inside_die(center_x, center_y):
                            ...
                            if DEBUG and k==0 and i==self.geo.nx//2 and j==self.geo.ny//2:
